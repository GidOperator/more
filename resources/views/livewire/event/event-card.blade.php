    ...
    @if ($view === 'grid')
        <div class="events-col">
            <div class="event-card">
                <div class="event-card__top-btns">
                    <a href="#" role="button"><span class="icon-visibility"></span></a>
                    <button type="button" wire:click="toggleFavorite"
                        style="border:none; background:none; cursor:pointer;">
                        <span class="icon-bookmark" style="color: {{ $isFavorite ? '#ff4081' : 'inherit' }}"></span>
                    </button>
                    <a href="#" role="button"><span class="icon-share"></span></a>
                </div>

                <div class="event-card__carousel position-relative" style="height:180px; overflow:hidden;"
                    x-data="{ index: 0, images: @js($event->images->pluck('path')), next() { this.index = (this.index + 1) % this.images.length }, prev() { this.index = (this.index - 1 + this.images.length) % this.images.length } }">
                    <img :src="images.length ? '{{ asset('storage') }}/' + images[index] : '{{ asset('images/no-image.jpg') }}'"
                        class="card-img-top" style="height:180px; object-fit:cover;" alt="{{ $event->name }}">
                    <div class="slider-buttons">
                        <template x-if="images.length > 1"><button @click.stop="prev" class="slider-arrow --prev"><span
                                    class="icon-chevron-lft"></span></button></template>
                        <template x-if="images.length > 1"><button @click.stop="next" class="slider-arrow --nex"><span
                                    class="icon-chevron-rght"></span></button></template>
                    </div>
                </div>

                <div class="event-card__header">
                    <div class="event-card__user">
                        <div class="event-card__user-avatar"><img src="{{ asset($event->organizer->logo) }}"
                                alt="{{ $event->organizer->name }}"></div>
                        <div class="event-card__user-name">{{ $event->organizer->user->name }}</div>
                    </div>
                    <div class="event-card__raiting">
                        <span class="event-card__raiting-star"><img src="{{ asset('images/raiting-star.svg') }}"
                                alt=""></span>
                ...
                <div class="event-card__body d-flex flex-column">
                    <h2 class="event-card__title"><a href="{{ route('event.show', $event) }}" wire:navigate
                            class="event-card__link">{{ $event->name }}</a></h2>
                    <div class="event-card__start">
                        <div class="event-card__start-date">{{ $event->date_start->format('d.m.Y') }}</div>
                        <div class="event-card__start-time">{{ $event->time_start }}</div>
                        <div class="event-card__start-duration">{{ $event->duration_minutes }} часов</div>
                    </div>
                    <div class="event-card__adress">
                        {{ $event->city?->name }}{{ $event->address ? ', ' . $event->address : '' }}</div>

                    <div class="event-card__footer w-100">

                        <div class="d-flex justify-content-between align-items-center w-100 mb-2">
                            <div class="event-card__categories d-flex gap-2">
                                <a href="#" class="event-card__category"><svg width="11" height="11"
                                ...
                                <a href="#" class="event-card__category"><svg width="11" height="10"
                                ...
                                <a href="#" class="event-card__category"><svg width="15" height="12"
                            ...
                            <div class="event-card__price">
                                <span class="old-price">20 000
                                    ₽</span><span>{{ number_format($event->price, 0, '.', ' ') }}</span> ₽
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center w-100">
                            ...
                            <div class="d-flex align-items-center" style="gap: 8px;">
                                <div class="event-card__limit small">
                                    до <span class="event-card__limit-value">{{ $event->limit }}</span> человек
                                </div>

                                <a href="#"
                                    class="event-card__comments d-flex align-items-center text-decoration-none"
                                    style="gap: 2px;">
                                    ...
                                    <div class="event-card__comments-value">120</div>
                                </a>
                            </div>
        ...
        <div class="list-group-item mb-3 shadow-sm">
            <div class="row g-3 align-items-center">
                ...
                <div class="col">
                    ...
                    <small class="text-muted">{{ $event->category->name }} /
                        {{ $event->subCategory->name ?? '' }}</small>
                    <p>{{ Str::limit($event->description, 140) }}</p>
                    <div class="d-flex align-items-center gap-3">
                        <button type="button" wire:click="toggleFavorite" style="border:none; background:none;">
                            <span class="icon-bookmark"
                                style="color: {{ $isFavorite ? '#ff4081' : 'inherit' }}"></span>
                        </button>
                        <span>{{ $event->date_start->format('d.m.Y') }} | {{ $event->time_start }}</span>
                    </div>
                </div>
                <div class="col-12 col-md-2 text-md-end">
                    <div class="fw-bold text-primary">{{ number_format($event->price, 0, '.', ' ') }} ₽</div>
                    <div class="text-muted"><s>20 000 ₽</s></div>
                </div>
            </div>
        </div>
    @endif
